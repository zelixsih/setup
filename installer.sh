...
create_mass_allocations() {
  ...
  # Auto baca konfigurasi login database dari file .env milik panel lu
  DB_NAME=$(grep "^DB_DATABASE=" "${PANEL_DIR}/.env" | cut -d'=' -f2 | tr -d '"' | tr -d "'")
  DB_USER=$(grep "^DB_USERNAME=" "${PANEL_DIR}/.env" | cut -d'=' -f2 | tr -d '"' | tr -d "'")
  DB_PASS=$(grep "^DB_PASSWORD=" "${PANEL_DIR}/.env" | cut -d'=' -f2 | tr -d '"' | tr -d "'")
  DB_HOST=$(grep "^DB_HOST=" "${PANEL_DIR}/.env" | cut -d'=' -f2 | tr -d '"' | tr -d "'")

  # Proses Input Data Allocations
  read -p "ID NODES (default 1): " NODE_ID
  read -p "IP ADDRESS (0.0.0.0): " IP_ADDR
  IP_ADDR=${IP_ADDR:-0.0.0.0}
  
  read -p "NAME IP ALIAS : " IP_ALIAS
  read -p "PORT MULAI (2000)    : " PORT_START
  read -p "PORT SELESAI (2900)    : " PORT_END

  # Validasi Angka Angka inputan
  if [[ ! "$NODE_ID" =~ ^[0-9]+$ ]] || [[ ! "$PORT_START" =~ ^[0-9]+$ ]] || [[ ! "$PORT_END" =~ ^[0-9]+$ ]]; then
    echo -e "${RED}[X] Error: Node ID, Port Start, dan Port End harus berupa ANGKA!${NC}"
    sleep 2
    return
  fi
  ...
  echo -e "\n${YELLOW}[*] Sedang men-generate ${TOTAL_PORTS} port (${PORT_START} - ${PORT_END}) mohon tunggu...${NC}"
  ...
  if [ $? -eq 0 ]; then
    ...
    echo -e "${GREEN}[+] =============================================== [+]${NC}"
    echo -e " Port ${PORT_START} sampai ${PORT_END} berhasil dimasukkan ke Node ID ${NODE_ID}."
  else
    echo -e "${RED}[X] Gagal memasukkan data ke database. Cek detail login MySQL atau ID Node lu!${NC}"
  fi
