...
# 🚀 FUNGSI AUTO CREATE ALLOCATIONS MASSAL (VERSI ADMIN API - CLEAN VERSION)
create_mass_allocations() {
  echo " "
  echo -e "${BLUE}[+] ========================================== [+]${NC}"
  echo -e "${BLUE}[+]           AUTO CREATE ALLOCATIONS          [+]${NC}"
  echo -e "${BLUE}[+] ========================================== [+]${NC}"
  echo " "

  # 1. Validasi Keberadaan File .env Panel
  if [ ! -f "${PANEL_DIR}/.env" ]; then
    echo -e "${RED}[X] Error: File .env Pterodactyl tidak ditemukan di ${PANEL_DIR}!${NC}"
    echo -e "${YELLOW}Pastikan Pterodactyl Panel sudah terinstall di VPS ini.${NC}"
    read -p "Tekan Enter untuk kembali..."
    return
  fi

  # 2. Ambil Kredensial API & URL Otomatis dari .env
  PTLA_KEY=$(grep "^APP_SERVICE_AUTHORIZATION=" "${PANEL_DIR}/.env" | cut -d'=' -f2 | sed "s/['\"]//g" | tr -d ' ')
  PANEL_URL=$(grep "^APP_URL=" "${PANEL_DIR}/.env" | cut -d'=' -f2 | sed "s/['\"]//g" | tr -d ' ')

  # Validasi jika API Key kosong/null
  if [ -z "$PTLA_KEY" ] || [ -z "$PANEL_URL" ] || [ "$PTLA_KEY" == "null" ]; then
    echo -e "${RED}[X] Error: API Key (APP_SERVICE_AUTHORIZATION) tidak ditemukan di .env!${NC}"
    echo -e "${YELLOW}Silakan buat Application API Key (Read & Write Admin) dulu di panel lu.${NC}"
    read -p "Tekan Enter untuk kembali..."
    return
  fi
  ...
  # 3. Input Data Node & Validasi Angka
  read -p "Masukkan ID Node yang mau diisi (Cek di panel, ex: 3): " NODE_ID
  if [[ ! "$NODE_ID" =~ ^[0-9]+$ ]]; then
    echo -e "${RED}[X] Error: ID Node harus berupa ANGKA!${NC}"
    sleep 2
    return
  fi

  # 4. Input Data Port & Konfigurasi
  read -p "Masukkan IP Address (Default: 0.0.0.0)      : " IP_ADDR
  IP_ADDR=${IP_ADDR:-0.0.0.0}
  
  read -p "Masukkan Nama/IP Alias (Boleh kosong)      : " IP_ALIAS
  read -p "Masukkan Port Mulai (Awal, ex: 2000)       : " PORT_START
  read -p "Masukkan Port Selesai (Akhir, ex: 2900)     : " PORT_END

  # Validasi Input Angka Port
  if [[ ! "$PORT_START" =~ ^[0-9]+$ ]] || [[ ! "$PORT_END" =~ ^[0-9]+$ ]]; then
    echo -e "${RED}[X] Error: Port Start dan Port End harus berupa ANGKA!${NC}"
    sleep 2
    return
  fi

  if [ "$PORT_START" -gt "$PORT_END" ]; then
    echo -e "${RED}[X] Error: Port mulai tidak boleh lebih besar dari port selesai!${NC}"
    sleep 2
    return
  fi
  ...
  echo -e "\n${YELLOW}[*] Men-generate ${TOTAL_PORTS} port (${PORT_START} - ${PORT_END}) via Admin API ke Node ID ${NODE_ID}...${NC}"
  echo -e "-----------------------------------------------------"
  
  # Set nilai Alias untuk JSON format
  if [ -z "$IP_ALIAS" ]; then
    ALIAS_VALUE="null"
  else
    ALIAS_VALUE="\"$IP_ALIAS\""
  fi

  # Mastiin tools pendukung (jq) terinstall buat baca feedback panel
  if ! command -v jq &> /dev/null; then
    sudo apt-get install jq -y &>/dev/null
  fi

  # 🚀 6. PROSES INJEKSI ALLOCATIONS VIA API LAYER
  local port
  local count=1
  local progress_interval=$((TOTAL_PORTS / 10 + 1)) # Nampilin progress update per 10%

  for ((port=PORT_START; port<=PORT_END; port++)); do
    
    # Bikin Struktur Data JSON pack buat dilempar ke API
    local JSON_BODY="[{\"ip\":\"${IP_ADDR}\",\"ip_alias\":${ALIAS_VALUE},\"port\":\"${port}\"}]"
    
    # Eksekusi Tembakan Curl API Resmi Pterodactyl
    local RESPONSE=$(curl -s -X POST "${PANEL_URL}/api/application/nodes/${NODE_ID}/allocations" \
      -H "Authorization: Bearer ${PTLA_KEY}" \
      -H "Content-Type: application/json" \
      -H "Accept: application/json" \
      -d "${JSON_BODY}")

    # Cek apakah ada feedback eror dari sistem API
    local ERR_CHECK=$(echo "$RESPONSE" | jq -r '.errors // empty')

    if [ ! -z "$ERR_CHECK" ]; then
        # Jika eror (misal ID Node gak ketemu atau port udah ada/duplikat)
        local ERROR_MSG=$(echo "$RESPONSE" | jq -r '.errors[0].detail // empty')
        echo -e "${RED}[X] Port ${port} GAGAL: ${ERROR_MSG}${NC}"
  	fi

    # Cetak status progress biar lu tau script-nya gak hang
    if (( count % progress_interval == 0 )); then
        local percentage=$(( count * 100 / TOTAL_PORTS ))
        echo -e "${CYAN}[*] Progress Berjalan: ${count} / ${TOTAL_PORTS} Port (${percentage}%) Selesai...${NC}"
    fi
    
    # Kasih jeda mikro (0.05 detik) biar request API teratur dan gak kena rate-limit nginx
    sleep 0.05
    count=$((count+1))
  done
  ...
  echo -e "-----------------------------------------------------"
  echo -e "${GREEN}[+] ======================== ============ [+]${NC}"
  echo -e "${GREEN}[+]       AUTO CREATE ALLOCATIONS       [+]${NC}"
  echo -e "${GREEN}[+] ========================= =========== [+]${NC}"
  echo -e " Port ${PORT_START} sampai ${PORT_END} selesai diproses untuk Node ID ${NODE_ID}."
  echo " "
  read -p "Tekan Enter untuk kembali ke menu utama..."
}
...
while true; do
  ...
  echo -e "  ${GREEN}11.${NC} Auto Create Allocations"
