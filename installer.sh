...
create_mass_allocations() {
  ...
  DB_NAME=$(grep "^DB_DATABASE=" "${PANEL_DIR}/.env" | cut -d'=' -f2 | sed "s/['\"]//g" | tr -d ' ')
  DB_USER=$(grep "^DB_USERNAME=" "${PANEL_DIR}/.env" | cut -d'=' -f2 | sed "s/['\"]//g" | tr -d ' ')
  DB_PASS=$(grep "^DB_PASSWORD=" "${PANEL_DIR}/.env" | cut -d'=' -f2 | sed "s/['\"]//g" | tr -d ' ')
  DB_HOST=$(grep "^DB_HOST=" "${PANEL_DIR}/.env" | cut -d'=' -f2 | sed "s/['\"]//g" | tr -d ' ')
  ...
  echo -e "\n${YELLOW}[*] Menghubungi database untuk memverifikasi ID Node ${NODE_ID}...${NC}"

  CHECK_NODE=$(mysql -h "$DB_HOST" -u "$DB_USER" -p"$DB_PASS" "$DB_NAME" -N -e "SELECT id FROM nodes WHERE id=$NODE_ID LIMIT 1;" 2>/dev/null)

  if [ -z "$CHECK_NODE" ]; then
    echo -e "${RED}[X] Error: ID Node ${NODE_ID} TIDAK DITEMUKAN di database panel lu!${NC}"
    echo -e "${YELLOW}Buka panel lu, masuk menu Nodes, dan cek angka ID yang bener di samping namanya.${NC}"
    read -p "Tekan Enter untuk kembali..."
    return
  fi

  echo -e "${GREEN}[✓] ID Node ${NODE_ID} valid. Melanjutkan input port...${NC}\n"

  # 5. Input Data Port
  read -p "Masukkan IP Address (Default: 0.0.0.0)      : " IP_ADDR
  IP_ADDR=${IP_ADDR:-0.0.0.0}
  
  read -p "Masukkan Nama/IP Alias (ex: Masukkan Name) : " IP_ALIAS
  ...
  echo -e "\n${YELLOW}[*] Sedang men-generate ${TOTAL_PORTS} port (${PORT_START} - ${PORT_END}) ke ID Node ${NODE_ID}...${NC}"

  if [ -z "$IP_ALIAS" ]; then
    ALIAS_VALUE="NULL"
  else
    # Tambahkan tanda petik tunggal untuk teks SQL
    ALIAS_VALUE="'$IP_ALIAS'"
  fi

  SQL_QUERY="INSERT INTO allocations (node_id, ip, ip_alias, port, server_id, created_at, updated_at) VALUES "
  ...
  for ((port=PORT_START; port<=PORT_END; port++)); do
    ...
    SQL_QUERY+="($NODE_ID, '$IP_ADDR', $ALIAS_VALUE, $port, NULL, '$current_time', '$current_time')"
  done

  SQL_QUERY+=" ON DUPLICATE KEY UPDATE updated_at=updated_at;"

  mysql -h "$DB_HOST" -u "$DB_USER" -p"$DB_PASS" "$DB_NAME" -e "$SQL_QUERY" 2>/dev/null

  if [ $? -eq 0 ]; then
    echo -e "                                                       "
    echo -e "${GREEN}[+] =============================================== [+]${NC}"
    echo -e "${GREEN}[+]   SUKSES MENAMBAHKAN ${TOTAL_PORTS} ALLOCATIONS PORTS!   [+]${NC}"
    echo -e "${GREEN}[+] =============================================== [+]${NC}"
    echo -e " Port ${PORT_START} sampai ${PORT_END} berhasil diinjeksi ke Node ID ${NODE_ID}."
  else
    echo -e "${RED}[X] Gagal memasukkan data ke database secara instan.${NC}"
    echo -e "${YELLOW}Coba periksa file .env lu manual. Username '${DB_USER}' atau Host '${DB_HOST}' atau DB '${DB_NAME}' mungkin salah.${NC}"
    echo -e "${YELLOW}Atau user MySQL lu gak punya permission buat INSERT ke tabel allocations.${NC}"
  fi
