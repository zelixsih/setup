...
show_banner() {
  ...
  # Menyusun string uptime secara dinamis dan bersih
  local uptime_vps=""
  if [ $days -gt 0 ]; then 
    uptime_vps+="${days} Hari "
  fi
  if [ $hours -gt 0 ] || [ $days -gt 0 ]; then 
    uptime_vps+="${hours} Jam "
  fi
  uptime_vps+="${mins} Menit"

  # 2. Ambil Data Sistem Otomatis
  local ip_vps=$(hostname -I | awk '{print $1}')
  local os_version=$(hostnamectl | grep "Operating System" | cut -d: -f2 | sed 's/^[ \t]*//')
  [ -z "$os_version" ] && os_version="Linux (Ubuntu 22.04)"
  
  local arch_vps=$(uname -m)
  local hostname_vps=$(hostname)
  
  # Deteksi Provider Otomatis
  local provider_vps=$(systemd-detect-virt 2>/dev/null | tr 'a-z' 'A-Z')
  [ -z "$provider_vps" ] || [ "$provider_vps" = "NONE" ] && provider_vps=$(cat /sys/class/dmi/id/sys_vendor 2>/dev/null)
  [ -z "$provider_vps" ] && provider_vps="KVM"

  # Ambil nama Model Prosedor / CPU info
  ...
  # 3. Cetak Logo (Kiri) dan Systems Status Lengkap dgn Prosedor (Kanan)
  echo -e "${RED}        _,gggggggggg.      ${LIGHT_BLUE}𝚂𝚈𝚂𝚃𝙴𝙼𝚂 𝚂𝚃𝙰𝚃𝚄𝚂${NC}"
  echo -e "${RED}    ,ggggggggggggggggg.    ${CYAN}⌯ 𝙾𝚂       :${NC} ${WHITE}${os_version}${NC}"
  echo -e "${RED}  ,ggggg        gggggggg.  ${CYAN}⌯ 𝙿𝚕𝚊𝚝𝚏𝚘𝚛𝚖 :${NC} ${WHITE}𝙻𝙸𝙽𝚄𝚇${NC}"
  echo -e "${RED} ,ggg'               'ggg. ${CYAN}⌯ 𝙲𝙿𝚄 𝙼𝚘𝚍𝚎𝚕:${NC} ${MAGENTA}${cpu_model}${NC}"
  echo -e "${RED}',gg       ,ggg.      'ggg:${CYAN}⌯ 𝙰𝚛𝚌𝚑     :${NC} ${WHITE}${arch_vps}${NC}"
  echo -e "${RED}'ggg      ,gg'''  .    ggg ${CYAN}⌯ 𝙷𝚘𝚜𝚝𝚗𝚊𝚖𝚎 :${NC} ${YELLOW}${hostname_vps}${NC}"
  echo -e "${WHITE}gggg      gg     ,     ggg ${CYAN}⌯ 𝚁𝚎𝚐𝚒𝚘𝚗   :${NC} ${GREEN}𝙹𝚊𝚔𝚊𝚛𝚝𝚊${NC}"
  echo -e "${WHITE}ggg:     gg.     -   ,ggg  ${CYAN}⌯ 𝙿𝚛𝚘𝚟𝚒𝚍𝚎𝚛 :${NC} ${BLUE}${provider_vps}${NC}"
  ...
  # 4. Menu Monitor Resource di Bawah Logo (Simpel & Padat)
  echo -e "${R1}┌ 𝚄𝙿𝚃𝙸𝙼𝙴 𝚅𝙿𝚂       : ${uptime_vps}${NC}"
  echo -e "${R2}├ 𝙸𝙿 𝚅𝙿𝚂           : ${ip_vps}${NC}"
  echo -e "${R3}├ 𝙲𝙿𝚄 𝙲𝙾𝚁𝙴𝚂     : ${cpu_cores} Core(s) Active${NC}"
  echo -e "${R5}├ 𝚁𝙰𝙼 𝚂𝚃𝙰𝚃𝚄𝚂    : ${ram_used} / ${ram_total}${NC}"
  echo -e "${R6}└ 𝙳𝙸𝚂𝙺 𝚂𝙿𝙰𝙲𝙴   : ${disk_usage} / ${disk_total}${NC}"
  echo -e ""
