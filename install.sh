...
setup_multi_nodes() {
  ...
  # --- FIX 1: VALIDASI PING DNS SEBELUM LIAR TEMBAK CERTBOT ---
  ...
  # Buat direktori volume khusus node ini biar ga bentrok
  sudo mkdir -p /var/lib/pterodactyl${NODE_SUFFIX}/volumes

  # --- FIX 2: PEMISAHAN DIREKTORI VOLUME ---
...
EOF
  ...
  echo -e "                                                       "
  echo -e "${GREEN}[+] =============================================== [+]${NC}"
  echo -e "${GREEN}[+]             SETUP NODES SELESAI SCRIPT!          [+]${NC}"
  echo -e "${GREEN}[+] =============================================== [+]${NC}"
...
while true; do
  ...
  # Cetak Logo Lama + Status Sistem Kanan Secara Presisi
  show_legacy_neofetch
  ...
  echo -e " ------------------------------------------------------------"
  echo -e " ${YELLOW}Masukkan pilihan:${NC}"
  read -r MENU_CHOICE
  clear
